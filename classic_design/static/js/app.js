...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Allow page scroll when pointer is over chart areas (Chart.js/canvas often consumes wheel)
    setupChartAreaScrollPassthrough();
});

/**
 * Two-finger / wheel scroll passthrough for the whole page.
 * Chart.js and canvas capture wheel events and prevent default, so touchpad scroll
 * often fails when the pointer is over the graphs. We run in capture phase and
 * scroll the window for any wheel inside the main .container, then prevent the
 * event from reaching the chart so the page always scrolls.
 */
function setupChartAreaScrollPassthrough() {
    var scrollLineHeight = 40; // px per "line" when deltaMode is DOM_DELTA_LINE (Mac touchpad)
    document.addEventListener('wheel', function(e) {
        var el = e.target;
        if (!el || !el.closest) return;
        if (!el.closest('.container')) return;
        // Normalize delta: Mac touchpad often uses deltaMode 1 (lines), browser uses 0 (pixels)
        var dy = e.deltaY;
        var dx = e.deltaX;
        if (e.deltaMode === 1) {
            dy = dy * scrollLineHeight;
            dx = dx * scrollLineHeight;
        } else if (e.deltaMode === 2) {
            dy = dy * (scrollLineHeight * 8);
            dx = dx * (scrollLineHeight * 8);
        }
