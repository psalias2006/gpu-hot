...
def monitor_loop(socketio, monitor):
    ...
    uses_nvidia_smi = (
        any(monitor.use_smi.values()) if hasattr(monitor, "use_smi") else False
    )
    update_interval = (
        config.NVIDIA_SMI_INTERVAL if uses_nvidia_smi else config.UPDATE_INTERVAL
    )
    ...
    while monitor.running:
        try:
            ...
            socketio.emit(
                "gpu_data",
                {"gpus": gpu_data, "processes": processes, "system": system_info},
                namespace="/",
            )
